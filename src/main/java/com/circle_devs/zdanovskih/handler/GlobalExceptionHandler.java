...
/**
 * Exception handler ensures that the error text
 * is clearly displayed to the user and in the log
 */
@Slf4j
@ControllerAdvice
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleConstraint(DataIntegrityViolationException ex) {
        ...
        if (msg.contains("Duplicate")
                || msg.contains("повторяющееся значение ключа нарушает ограничение уникальности")
                || msg.contains("duplicate key value violates unique constraint")) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body("Data already exists");
            }
        if (msg.contains("foreign key")
                || msg.contains("нарушает ограничение внешнего ключа")) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
    ...
    /**
     * Log & return all other unknown errors
